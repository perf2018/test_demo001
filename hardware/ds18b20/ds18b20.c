...
extern void ds_output_state(int);				//控制某一个GPIO输出高电平 / 低电平
extern int ds_input_state(void);			//获取GPIO的状态(1高电平	0低电平)
//需要自己实现PB15
...
{ 
	...
	ds_output_state(0);					//首先给GPIO输出低电平
	//延时 480 微秒,产生复位脉冲 
	delay_us(480);
	ds_output_state(1);					//给GPIO输出高电平
	//延时 80 微秒对总线采样 
	delay_us(80);
	flag = ds_input_state();   
	//对数据脚采样 
...
{ 
	...
	for(i = 8; i > 0; i--){
		ds_output_state(0);                  //拉低总线,产生写信号 
		delay_us(4);            		//延时 4us 
		ds_output_state(wData & 0x01); 	//发送 1 位 (我们的传感器是先低位后高位)
		delay_us(60);          		//延时 60us,写时序至少要 60us 
		ds_output_state(1);                  //释放总线,等待总线恢复 
		wData >>= 1;                    //准备下一位数据的传送 
	} 
} 
...
{ 
	...
	DS1820_WriteData(0x7F);    //  将配置寄存器配置为 12 位精度 手册P8
...
{ 
	...
	for(i = 8; i > 0; i--){
		TmepData >>= 1; 
		ds_output_state(0);          //拉低总线,产生读信号 
		delay_us(4);              	//延时 4us 
		ds_output_state(1);          //释放总线,准备读数据 
		delay_us(8);              	//延时 8 微秒读数据 
		if(ds_input_state())
			TmepData |= 0x80; 				//按从低位到高位的顺序发送数据
		delay_us(60);            		//延时 60us 
		ds_output_state(1);                  //拉高总线,准备下一位数据的读取. 
	} 
...
例子
1111 = 15
2^3 + 2^2 + 2^1 + 2^0 = 15; <===> (2^3 + 2^2 + 2^1 + 2^0) * 1
(这里每一位的权值都是1)
如果权值换成0.0625
(2^3 + 2^2 + 2^1 + 2^0) * 0.0625 = 15 * 0.0625
*/
